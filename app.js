...
import { db, app } from "./firebase-core.js";
import {
...
// ---- Push bildirimleri ----
// BURAYI DEGISTIR: Firebase Console > Project Settings > Cloud Messaging >
// Web Push certificates kisminda "Generate key pair" ile olusan ANAHTARI
// buraya yapistir. Girilmeden push bildirimleri calismaz (sessizce devre disi kalir).
const FCM_VAPID_KEY = "BBMfrXJ6VxseQgZJY9z-gAeFAWDKkI5Uac0K58bjLfJfZ3aZWDXxQvbZpvFxT-CDu_bh-73jARGX1uJxMuVTdGQ";

var pushMessaging = null;
try { pushMessaging = getMessaging(app); } catch (e) { console.error("[cariTakip] messaging baslatilamadi:", e); }

async function setupPushNotifications(){
  if (!pushMessaging) return;
  if (!("Notification" in window) || !("serviceWorker" in navigator)) return;
  if (FCM_VAPID_KEY.indexOf("PASTE_") === 0){
    console.warn("[cariTakip] VAPID anahtari henuz girilmedi, push bildirimleri devre disi.");
    return;
  }
  try {
    ...
    var token = await getToken(pushMessaging, { vapidKey: FCM_VAPID_KEY, serviceWorkerRegistration: reg });
    if (!token) return;

    await setDoc(doc(db, "pushTokens", docIdFor(token)), {
      token: token,
      role: currentRole,
      isim: currentRole === "admin" ? currentUserName : (currentPlakalar[0] || ""),
      updatedAt: serverTimestamp()
    }, { merge: true });
  } catch (e) {
    console.error("[cariTakip] push kaydi basarisiz:", e);
  }
}
...
function wireHamburger(){
  ...
  on("sendDuyuruBtn", "click", async function(){
    var title = document.getElementById("duyuruTitleInput").value.trim();
